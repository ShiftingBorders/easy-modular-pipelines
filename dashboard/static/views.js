...
export function metricCards(rows, selections, forecast = null, summaries = null, cycleCount = null) {
    ...
    return `<div class="context"><span>Statistics unit: <b>1 DAG cycle</b></span><span>Sample: <b>${cycleCount ?? new Set(usable.map(row => row.cycle_number)).size} completed cycles</b></span></div><div class="metric-grid">${[0, 1, 2].map(index => {
        ...
        const summary = summaries?.find(row => `${row.module_name} / ${row.module_version || "?"}` === module && row.metric === metric);
        const mean = summary ? summary.mean : compatible && known.length === selected.length ? known.reduce((sum, row) => sum + row.value, 0) / known.length : null;
        const maximum = Math.max(1e-10, ...known.map(row => Math.abs(row.value)));
        const unit = summary ? summary.unit : units.size === 1 ? [...units][0] : "Mixed units";
        const incomplete = summary ? summary.incomplete : !compatible || known.length !== selected.length || selected.some(row => row.complete === false || row.estimated);
        const additive = summary ? summary.additive : selected.every(row => row.aggregation === "sum");
...
export function forecast(document, selections) {
    ...
    const componentChart = components.length ? `<div class="panel-body">${components.map(row => `<div class="component-duration" data-key="component:${e(row.stage_id)}"><span>${e(row.module_name)}</span><div class="bar-track"><i style="width:${Math.max(0, (row.mean_seconds || 0) / maximum * 100)}%"></i></div><strong>${duration(row.mean_seconds)}</strong></div>`).join("")}</div>` : empty("No completed cycles");
...
export function parameters(rows) {
    return table(rows, [["Attempt", row => inspectButton(row.attempt_id, row)], ["Module", row => e(row.module_name)], ["Cycle", row => numeric(row.cycle_number)]], [], "attempts");
}
